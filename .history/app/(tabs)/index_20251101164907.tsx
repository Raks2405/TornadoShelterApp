...
const App: React.FC = () => {
  ...
  const bottomSheetAnim = useRef<Animated.Value>(new Animated.Value(150)).current;
  ...
  const getThreatText = (probability: number) => {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {shelters.map((shelter) => {
          const occupancy =
            parseInt(shelter.occupants.split('/')[0]) /
            parseInt(shelter.occupants.split('/')[1]);
      ...
      {/* The rest of your UI (weather stats, compass, bottom sheet, modal) */}
      {/* ---- KEEP YOUR ORIGINAL ---- */}
    </View>
  );
};
