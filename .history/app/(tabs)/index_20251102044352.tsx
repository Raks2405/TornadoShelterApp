...
export default function App() {
  ...
  useEffect(() => {
    // Re-render weather UI when tick changes
    console.log("🌪️ UI refreshed, new data:", weatherData);
  }, [updateTick]);
  ...
  useEffect(() => {
    const getWeather = async () => {
      ...
      if (!data) return;
      if ((data.threat === "HIGH" || data.threat === "SEVERE") && lastAlertRef.current !== data.threat) {
        lastAlertRef.current = data.threat;
