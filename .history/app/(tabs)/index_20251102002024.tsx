...
const ShelterMarker = ({ s, onPress }: { s: Shelter; onPress: (s: Shelter) => void }) => {
  ...
  return (
    <Marker
      ...
      anchor={{ x: 0.5, y: 0.5 }}
      calloutAnchor={{ x: 0.5, y: 1.2 }} // push callout above icon
      onPress={() => {
        // On some Android devices, callout won't open unless you handle onPress
        if (isAndroid) onPress(s);
      }}
    >
      {/* Home icon marker */}
      <View
      ...
      {/* Show callout on iOS (Android tooltips are flaky) */}
      {!isAndroid && (
        <Callout tooltip={false} onPress={() => {
          console.log("Tapped:", s.name);
          onPress(s)}
