...
const ShelterMarker = ({ s, onPress }: { s: Shelter; onPress: (s: Shelter) => void }) => {
  ...
  return (
    <Marker
      ...
      // ❌ Do not pass title/description when using a custom Callout on Android
      tracksViewChanges={!freeze}
      // Optional: improves tap hitbox on Android
      anchor={{ x: 0.5, y: 0.5 }}
      calloutAnchor={{ x: 0.5, y: 0 }}
    >
      {/* Custom marker icon (single View child) */}
      <View
        onLayout={() => {
          if (!didLayout.current) {
            didLayout.current = true;
            // freeze after first layout to avoid constant re-draws
            setTimeout(() => setFreeze(true), 300);
          }
        }}
        style={{
          width: 34,
      ...
      {/* Custom callout. Use tooltip to fully control content on Android */}
      <Callout tooltip onPress={() => onPress(s)}>
        <View
          style={{
            backgroundColor: "#fff",
            padding: 10,
        ...
        >
          <Text style={{ fontWeight: "700", marginBottom: 4, color: "#111" }}>{s.name}</Text>
          <Text style={{ color: "#333", fontSize: 12 }}>{s.address}</Text>
          <Text style={{ color: "#007AFF", fontSize: 11, marginTop: 6 }}>Tap for directions</Text>
        </View>
      </Callout>
    </Marker>
  );
