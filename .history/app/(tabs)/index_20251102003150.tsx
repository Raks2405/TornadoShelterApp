...
const ShelterMarker = ({ s, onPress }: { s: Shelter; onPress: (s: Shelter) => void }) => {
  ...
  return (
    <Marker
      ...
      anchor={{ x: 0.5, y: 0.5 }}
      onPress={() => setShowTooltip(!showTooltip)} // toggle popup manually
    >
      {/* Home Icon */}
      <View
        onLayout={() => {
      ...
      {/* Custom bubble tooltip — shown via React, not native callout */}
      {showTooltip && (
        <View
          style={{
            position: "absolute",
            bottom: 46, // position above icon
            backgroundColor: "#fff",
            padding: 10,
            borderRadius: 8,
            borderColor: "#ddd",
        ...
        >
          <Text style={{ fontWeight: "bold", color: "#111", marginBottom: 2 }}>{s.name}</Text>
          <Text style={{ color: "#333", fontSize: 12, textAlign: "center" }}>{s.address}</Text>
          <TouchableOpacity onPress={() => onPress(s)}>
            <Text style={{ color: "#007AFF", fontSize: 12, marginTop: 6 }}>
              Tap for directions
            </Text>
          </TouchableOpacity>
        </View>
