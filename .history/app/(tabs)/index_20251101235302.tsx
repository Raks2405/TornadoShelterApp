...
const ShelterMarker = React.memo(
  ({ s, onPress }: { s: Shelter; onPress: (s: Shelter) => void }) => (
    <Marker coordinate={{ latitude: s.latitude, longitude: s.longitude }}>
      <View style={styles.marker}>
        <Ionicons name="home" size={18} color="#fff" />
      </View>
      <Callout tooltip={false} onPress={() => onPress(s)}>
        <View style={{ maxWidth: 220 }}>
          <Text style={{ fontWeight: "bold", marginBottom: 4 }}>{s.name}</Text>
          <Text style={{ color: "#007AFF" }}>{s.address}</Text>
        </View>
      </Callout>
    </Marker>
  )
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setShelters(sheltersRaw); // use your full list here

      subscription = await Location.watchPositionAsync(
        { accuracy: Location.Accuracy.Lowest, timeInterval: 10000, distanceInterval: 30 },
        (pos) => {
  ...
  // ---- Compute distances only when location changes ----
  useEffect(() => {
