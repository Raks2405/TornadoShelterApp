...
const ShelterMarker = ({ s, onPress }: { s: Shelter; onPress: (s: Shelter) => void }) => {
  ...
  useEffect(() => {
    // Small delay to ensure callout renders properly
    const timer = setTimeout(() => setShowCallout(true), 100);
    ...
    >
      ...
      {showCallout && (
        <Callout onPress={() => onPress(s)}>
          <View style={{ width: 220, padding: 12 }}>
            <Text style={{ fontWeight: "bold", color: "#111", fontSize: 14 }}>
              {s.name}
            </Text>
            <Text style={{ color: "#555", fontSize: 12, marginTop: 2 }}>
              {s.address}
            </Text>
            <Text style={{ color: "#007AFF", fontSize: 11, marginTop: 6 }}>
              Tap for directions →
            </Text>
          </View>
        </Callout>
