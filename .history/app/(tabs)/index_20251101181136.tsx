...
const App: React.FC = () => {
  ...
  const getUserLocation = async () => {
    try {
      // Ask permission *politely*, but don’t block if denied
      const { status } = await Location.requestForegroundPermissionsAsync();

      if (status !== 'granted') {
        console.log('📍 Location permission denied — fallback to manual search');
        // Optional: You can show a toast or console message, but don’t alert.
        return; // just exit quietly
      }

      // Try to get location with a short timeout to avoid hanging
      ...
      console.log('⚠️ Unable to get location, fallback to search:', error.message);
      // Don’t block — let user manually search
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* HARD CODED DATA, NEED API TO IMPELEMENT THIS */}
        {shelters.map((s) => {
