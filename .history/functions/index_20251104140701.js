...
// Register endpoint → store token in Firestore
app.post("/register", async (req, res) => {
  try {
    ...
    // Save token in Firestore (merge to avoid overwrite)
...
// Manual notify endpoint → still works the same
...
export const tornadoScheduler = onSchedule(
  ...
  async () => {
    ...
    try {
      ...
      logger.info(`Found ${tokens.length} registered devices.`);

      if (probability >= 70 && tokens.length > 0) {
        logger.info(`⚠️ Sending alert to ${tokens.length} devices`);
