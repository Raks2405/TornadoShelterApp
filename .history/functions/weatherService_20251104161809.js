...
async function fetchTornadoIndicators(lat, lon) {
  const username = METEO_USERNAME.value();
  const password = METEO_PASSWORD.value();
  ...
  try {
    ...
    const pick = (param) => {
      ...
      return found?.coordinates?.[0]?.dates?.[0]?.value ?? null;
    };
    ...
    // 🌪️ continuous tornado risk formula
    const windFactor = Math.min(Math.max(wind / 30, 0), 1);
    const gustFactor = Math.min(Math.max(gusts / 40, 0), 1);
    const pressureFactor = Math.min(Math.max((1015 - pressure) / 20, 0), 1); // lower pressure = more risk
